    ...
    public void setBgmMusicSwitch(){
        bgmMusicSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isChecked){
                    mediaPlayer.loadResource("https://storage.zego.im/demo/201808270915.mp4", new IZegoMediaPlayerLoadResourceCallback() {
                        @Override
                        public void onLoadResourceCallback(int errorCode) {
                            if (errorCode == 0)
                            {
                                mediaPlayer.start();
                                AppLogger.getInstance().success("MediaPlayer load resource success.");
                            }
                            else
                            {
                                AppLogger.getInstance().fail("MediaPlayer load fail:%d", errorCode);
                            }
                        }
                    });
                } else {
