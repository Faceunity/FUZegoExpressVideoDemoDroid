package im.zego.advancedstreaming.streamByCdn;
...
public class DirectPublishingToCDNActivity extends AppCompatActivity {
        ...
        engine.setEventHandler(new IZegoEventHandler() {
            ...
            public void onPlayerStateUpdate(String streamID, ZegoPlayerState state, int errorCode, JSONObject extendedData) {
                ...
                if(errorCode != 0 && state.equals(ZegoPlayerState.NO_PLAY)) {
                    if (isPlay) {
                        playStreamFromUrlButton.setText(ZegoViewUtil.GetEmojiStringByUnicode(ZegoViewUtil.crossEmoji) + getString(R.string.stop_playing));
                    }
                } else {
                    if (isPlay) {
                        playStreamFromUrlButton.setText(ZegoViewUtil.GetEmojiStringByUnicode(ZegoViewUtil.checkEmoji) + getString(R.string.stop_playing));
                    }
                }
            }
        });
    ...
    protected void onResume() {
        ...
        if(isPublish && isEnablePublishDirectToCdn) {
            ZegoCDNConfig config = new ZegoCDNConfig();
            // set CDN URL
            config.url = publishCdnURL;
            engine.enablePublishDirectToCDN(true, config);
