package im.zego.advancedstreaming.publishingmultiplestreams.ui;
...
import im.zego.advancedstreaming.publishingmultiplestreams.camera.VideoCaptureFromImage2;
import im.zego.advancedstreaming.publishingmultiplestreams.camera.ZegoVideoCaptureCallback;
import im.zego.commontools.logtools.AppLogger;
import im.zego.commontools.logtools.LogView;
import im.zego.commontools.logtools.logLinearLayout;
import im.zego.commontools.uitools.ZegoViewUtil;
import im.zego.keycenter.UserIDHelper;
import im.zego.advancedstreaming.R;
        ...
        engine.setEventHandler(new IZegoEventHandler() {
            ...
            public void onPublisherStateUpdate(String streamID, ZegoPublisherState state, int errorCode, JSONObject extendedData) {
                ...
                if(errorCode != 0 && state.equals(ZegoPublisherState.NO_PUBLISH)) {
                    if (isPublishAux) {
                        publishAuxChannelButton.setText(ZegoViewUtil.GetEmojiStringByUnicode(ZegoViewUtil.crossEmoji) + getString(R.string.stop_publishing));
                    }
                } else {
                    if (isPublishAux) {
                        publishAuxChannelButton.setText(ZegoViewUtil.GetEmojiStringByUnicode(ZegoViewUtil.checkEmoji) + getString(R.string.stop_publishing));
                    }
                } else if (streamID.equals(publishMainStreamID)){
                    if(errorCode != 0 && state.equals(ZegoPublisherState.NO_PUBLISH)) {
                    if (isPublishMain) {
                        publishMainChannelButton.setText(ZegoViewUtil.GetEmojiStringByUnicode(ZegoViewUtil.crossEmoji) + getString(R.string.stop_publishing));
                    }
                } else {
                        if (isPublishMain) {
                            publishMainChannelButton.setText(ZegoViewUtil.GetEmojiStringByUnicode(ZegoViewUtil.checkEmoji) + getString(R.string.stop_publishing));
                        }
                    }
                }
            }
            ...
            public void onPlayerStateUpdate(String streamID, ZegoPlayerState state, int errorCode, JSONObject extendedData) {
                ...
                if (streamID.equals(playFirstStreamID)) {
                    if (errorCode != 0 && state.equals(ZegoPlayerState.NO_PLAY)) {
                        if (isPlayFirst) {
                            playFirstStreamButton.setText(ZegoViewUtil.GetEmojiStringByUnicode(ZegoViewUtil.crossEmoji) + getString(R.string.stop_playing));
                        }
                    } else {
                        if (isPlayFirst) {
                            playFirstStreamButton.setText(ZegoViewUtil.GetEmojiStringByUnicode(ZegoViewUtil.checkEmoji) + getString(R.string.stop_playing));
                        }
                    }
                } else if (streamID.equals(playSecondStreamID)){
                    if (errorCode != 0 && state.equals(ZegoPlayerState.NO_PLAY)) {
                        if (isPlaySecond) {
                            playSecondStreamButton.setText(ZegoViewUtil.GetEmojiStringByUnicode(ZegoViewUtil.crossEmoji) + getString(R.string.stop_playing));
                        }
                    } else {
                        if (isPlaySecond) {
                            playSecondStreamButton.setText(ZegoViewUtil.GetEmojiStringByUnicode(ZegoViewUtil.checkEmoji) + getString(R.string.stop_playing));
                        }
                    }
                }
            }
